...
// Generated 04-May-2022 12:57:43
  ...
  namespace matlab
  {
    ...
    void NodeInterface::initialize(int argc, char ** argv)
    {
      try {
        ...
        ROS_INFO("** Starting the model \"cbf_with_error\" **\n");

        // initialize the model which will initialize the publishers and subscribers
        rtmSetErrorStatus(cbf_with_error_M, (NULL));
        cbf_with_error_initialize();
    ...
    boolean_T NodeInterface::getStopRequestedFlag(void)
    {
      ...
      return (!(rtmGetErrorStatus(cbf_with_error_M)
                == (NULL)));

#else

      return (!(rtmGetErrorStatus(cbf_with_error_M)
                == (NULL)) || rtmGetStopRequested(cbf_with_error_M));
    ...
    {
      mRunModel = (rtmGetErrorStatus(cbf_with_error_M) ==
                   (NULL));
      while (mRunModel) {
        mBaseRateSem.wait();
